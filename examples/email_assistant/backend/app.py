...
async def process_email_stream(email_id: str):
    ...
    async def generate():
        ...
        try:
            ...
            # Accumulate step events. On resume, carry over prior steps (drop the
            # __stopped__ sentinel) so the full history accumulates correctly.
            completed_steps_log: list[dict] = (
                [s for s in email.get("completed_steps", []) if s.get("node") != "__stopped__"]
                if resuming else []
            )
